...
func metricWriter() {
	...
	defer ticker.Stop()
	batch := make([]HTTPRequestMetric, 0, 500)
	for {
		select {
...
func QueryPerformance(ctx context.Context, from, to time.Time, interval time.Duration) ([]PerformancePoint, PerformanceSummary, error) {
	...
	if !dbTo.Before(from.UTC()) {
		if err := db.ReadDB(ctx).Where("bucket_start >= ? AND bucket_start < ?", from.UTC(), dbTo).Order("bucket_start asc").Find(&rows).Error; err != nil {
