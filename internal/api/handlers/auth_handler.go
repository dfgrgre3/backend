...
func (h *AuthHandler) Logout(c *gin.Context) {
	...
	if exists {
		if jti, ok := jtiVal.(string); ok && jti != "" {
			...
			_ = tokenSvc.BlacklistJTI(jti, ttl)
		}
	}
...
func (h *AuthHandler) UnlinkProvider(c *gin.Context) {
	...
	var req dto.UnlinkProviderRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, http.StatusBadRequest, "Invalid input: "+err.Error())
		return
	}
...
func (h *AuthHandler) ValidateToken(c *gin.Context) {
	...
	if err != nil {
		response.Success(c, dto.ValidateTokenResponse{Valid: false})
		return
	}

	response.Success(c, dto.ValidateTokenResponse{
		Valid:       true,
...
func (h *AuthHandler) AccountRecovery(c *gin.Context) {
	...
	response.Success(c, dto.AccountRecoveryResponse{
		Ticket:  ticket,
