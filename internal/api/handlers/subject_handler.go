...
func GetHomepageData(c *gin.Context) {
	...
	readDB.Model(&models.Subject{}).Where("is_published = ? AND is_active = ?", true, true).Count(&totalCourses)

	var totalStudents int64
	readDB.Model(&models.User{}).Where("role = ?", models.RoleStudent).Count(&totalStudents)

	var totalTeachers int64
	readDB.Model(&models.User{}).Where("role = ?", models.RoleTeacher).Count(&totalTeachers)

	var totalEnrollments int64
	readDB.Model(&models.Enrollment{}).Count(&totalEnrollments)

	// Fetch featured teachers (limit 4)
	var featuredTeachers []models.User
	if err := readDB.Model(&models.User{}).
	...
	if db.Redis != nil {
		...
		go func(key string, payload []byte) {
			...
			defer cancel()
			db.Redis.Set(ctx, key, payload, cacheTTL)
		}(cacheKey, data)
	}
