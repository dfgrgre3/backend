...
// GetCourseStudents returns the list of enrolled students for a course (admin/teacher only)
...
func GetCourseStudents(c *gin.Context) {
	...
	// Filter by progress
	progressFilter := c.Query("progress") // "completed", "in_progress", "not_started"
	...
	for _, e := range enrollments {
		...
		if e.User.ID != "" {
			...
			studentInfo.Email = e.User.Email
			studentInfo.Avatar = e.User.Avatar
		}
...
func ManualEnroll(c *gin.Context) {
	...
	services.GetAuditService().LogAsync(input.UserID, "admin.enroll", "enrollment", subject.ID,
...
func UnenrollUser(c *gin.Context) {
	...
	services.GetAuditService().LogAsync(input.UserID, "admin.unenroll", "enrollment", subject.ID,
		map[string]interface{}{"adminId": c.GetString("userId")}, c.ClientIP(), c.Request.UserAgent())
