...
var allowedUploadCategories = map[string]uploadCategory{
	"image": {
		exts:  []string{".jpg", ".jpeg", ".png", ".gif", ".webp", ".avif", ".svg"},
		mimes: []string{"image/jpeg", "image/png", "image/gif", "image/webp", "image/avif", "image/svg+xml"},
	},
	"video": {
		exts:  []string{".mp4", ".mov", ".avi", ".mkv", ".webm"},
		mimes: []string{"video/mp4", "video/quicktime", "video/x-msvideo", "video/x-matroska", "video/webm"},
	},
	"audio": {
	...
	"any": {
		exts:  []string{".jpg", ".jpeg", ".png", ".gif", ".webp", ".avif", ".svg", ".mp4", ".mov", ".webm", ".mp3", ".wav", ".ogg", ".pdf", ".doc", ".docx", ".xls", ".xlsx"},
		mimes: []string{},
	},
}
...
func handleChunkedInit(c *gin.Context) {
	...
	category := "any"
	if !isExtAllowed(ext, category) {
		api_response.Error(c, http.StatusBadRequest, fmt.Sprintf("File extension '%s' is not allowed", ext))
		return
	}
