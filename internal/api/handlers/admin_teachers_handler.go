...
func GetTeacherApplications(c *gin.Context) {
	...
	if err := query.Model(&models.User{}).Count(&total).Error; err != nil {
		apiresponse.Error(c, http.StatusInternalServerError, "Failed to fetch teacher applications")
		return
	}
...
func ReviewTeacherApplication(c *gin.Context) {
	...
	if strings.TrimSpace(id) == "" {
		apiresponse.Error(c, http.StatusBadRequest, "Application id is required")
		return
	}

	var user models.User
	if err := database.Where("id = ? AND role = ?", id, models.RoleTeacher).First(&user).Error; err != nil {
		apiresponse.Error(c, http.StatusNotFound, "Application not found")
		return
	}
	...
	if err := database.Model(&models.User{}).Where("id = ?", user.ID).Update("instructor_status", newStatus).Error; err != nil {
		apiresponse.Error(c, http.StatusInternalServerError, "Failed to update application status")
		return
	}
