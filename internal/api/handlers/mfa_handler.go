...
type MFAHandler struct {
	...
	authRepo    services.AuthService // using to create sessions
}

func NewMFAHandler(mfaService services.MFAService, tokenSvc services.AuthTokenService, authService services.AuthService) *MFAHandler {
	return &MFAHandler{
...
func (h *MFAHandler) VerifyMFA(c *gin.Context) {
	...
	userID, err := db.Redis.Get(ctx, ticketKey).Result()
	if err != nil || userID == "" {
