...
func HandleWalletDeposit(c *gin.Context) {
	...
	if err := SafeCreate(db.DB, &payment); err != nil {
		...
		analyticsservice.GetAuditService().LogAsync(userId.(string), analyticsservice.AuditEventPaymentSuccess, "wallet_topup", payment.ID, map[string]interface{}{"amount": req.Amount}, c.ClientIP(), c.Request.UserAgent())
	}
