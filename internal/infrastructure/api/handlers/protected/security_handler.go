...
var (
	securityLogRepo     *authrepo.SecurityLogRepository
	securityLogRepoOnce sync.Once
)

func getSecurityLogRepo() *authrepo.SecurityLogRepository {
	securityLogRepoOnce.Do(func() {
		securityLogRepo = authrepo.NewSecurityLogRepository(db.DB)
	})
...
func LogSecurityEvent(userID string, eventType models.SecurityEventType, ip, userAgent string, location *string, metadata *string) error {
	...
	go func() {
		if err := getSecurityLogRepo().Create(logEntry); err != nil {
			log.Printf("[WARN] Failed to create security log: %v", err)
		}
	}()
	return nil
}
