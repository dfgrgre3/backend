...
func InitHub() {
	...
	GlobalHub.redisPubSub = cache.Redis.Subscribe(context.Background(), redisBroadcastChannel)
...
func (h *Hub) BroadcastToUser(userID string, message []byte) {
	...
	if cache.Redis != nil {
		...
		cache.Redis.Publish(context.Background(), redisBroadcastChannel, string(envelopeJSON))
	}
}
...
func (h *Hub) Run() {
	for {
		...
		case message := <-h.broadcast:
			...
			if cache.Redis != nil {
				cache.Redis.Publish(context.Background(), redisBroadcastChannel, string(message))
			}
		}
	}
}
