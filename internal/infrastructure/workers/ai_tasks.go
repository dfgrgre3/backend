...
func NewEssayGradeHandler() *EssayGradeHandler {
	return &EssayGradeHandler{aiService: aiservice.GetAIService()}
}
...
func NewLessonSummaryHandler() *LessonSummaryHandler {
	return &LessonSummaryHandler{aiService: aiservice.GetAIService()}
}
...
func setAIJobResult(ctx context.Context, prefix string, r AIJobResult) {
	...
	if err := cache.Redis.Set(ctx, prefix+r.JobID, data, aiJobResultTTL).Err(); err != nil {
...
func getAIJobResult(ctx context.Context, prefix, jobID string) (*AIJobResult, bool) {
	...
	raw, err := cache.Redis.Get(ctx, prefix+jobID).Result()
	if err != nil {
		return nil, false
	}
	var out AIJobResult
