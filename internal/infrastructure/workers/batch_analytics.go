...
func startListBasedWorkerWithContext(ctx context.Context) {
	...
	for {
		...
		result, err := cache.GetRedis().BRPop(ctx, 5*time.Second, analyticsStream).Result()
		if err != nil {
...
func ensureConsumerGroup() error {
	err := cache.GetRedis().XGroupCreateMkStream(
		context.Background(),
		analyticsStream,
...
func processBatch(ctx context.Context) error {
	...
	if err := cache.GetRedis().XAck(ctx, analyticsStream, analyticsConsumerGroup, ids...).Err(); err != nil {
...
func readAnalyticsGroupWithRetry(ctx context.Context, args *redis.XReadGroupArgs) ([]redis.XStream, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		entries, err := cache.GetRedis().XReadGroup(ctx, args).Result()
