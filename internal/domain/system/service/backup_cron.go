...
// NOTE: In production, prefer infrastructure-level backups (e.g. RDS Snapshots,
// Kubernetes CronJob). Set SKIP_AUTO_BACKUP=true to disable this in-app scheduler.
...
// SKIP_AUTO_BACKUP=true disables the in-app scheduler entirely.
func (w *BackupCronWorker) Start() {
	w.mu.Lock()
	defer w.mu.Unlock()

	if os.Getenv("SKIP_AUTO_BACKUP") == "true" {
		log.Println("[BackupCron] In-app backup scheduler disabled (SKIP_AUTO_BACKUP=true). Use infrastructure-level backups.")
		return
	}
